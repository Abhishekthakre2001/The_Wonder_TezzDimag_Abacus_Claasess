...
exports.addnewQ = async (req, res) => {
  try {
    ...
    const result = await QuestionPaperService.addnewQuestions({
      paperId,
      section,
      question_type,
      marks,
...
exports.getFilteredQuestionPapers = async (req, res) => {
  try {
    const { question_paper_type, level_id, set_id, paper_type } = req.query;
...
exports.deleteQuestionPaper = async (req, res) => {
  try {
    await QuestionPaperService.deleteQuestionPaper(req.params.id, req.user.id);
...
exports.exportQuestionPapers = async (req, res) => {
  try {
    ...
    await QuestionPaperService.exportQuestionPapers(req.user.id, search, res);
  } catch (err) {
    res.status(500).json({
      success: false,
      message: err.message,
